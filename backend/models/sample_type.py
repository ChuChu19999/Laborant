...
class SampleType(BaseModel):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, comment="Название типа пробы")
    ...
    laboratory: Mapped["Laboratory"] = relationship(back_populates="sample_types")
    department: Mapped["Department | None"] = relationship(back_populates="sample_types")
    ...
    __table_args__ = (
        ...
        Index("idx_sample_type_laboratory", "laboratory_id"),
        Index("idx_sample_type_department", "department_id"),
        {"schema": get_database_schema()},
    )

    def __repr__(self) -> str:
        return f"<SampleType(id={self.id}, name='{self.name}')>"
