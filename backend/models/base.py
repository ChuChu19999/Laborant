...
class BaseModel(Base):
    __abstract__ = True
    __table_args__: ClassVar[Any] = {"schema": get_database_schema()}
    ...
    def soft_delete(self) -> None:
        """Мягко удалить запись с установкой deleted_by, deleted_by_hash и deleted_at."""
...
def receive_before_insert(_mapper, _connection, target) -> None:
    """Автоматически заполнить created_by и created_by_hash при создании записи."""
    current_user = get_current_user()
...
def receive_before_update(_mapper, _connection, target) -> None:
    """Автоматически заполнить updated_by и updated_by_hash при обновлении записи."""
    current_user = get_current_user()
