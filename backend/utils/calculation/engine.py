...
def _eval_numeric_result(result: Any) -> Decimal:
    """Привести результат формулы к Decimal; иначе ValueError."""
...
def _round_to_multiple(number: Any, multiple: Any) -> Decimal:
    ...
    try:
        ...
        return quotient * m
    except (ArithmeticError, InvalidOperation, KeyError, TypeError, ValueError, ZeroDivisionError) as e:
        raise ValueError(f"Ошибка при округлении до кратного: {e!s}") from e


def round_value_by_params(
    value: Any,
    rounding_type: str | None = None,
    rounding_decimal: Any = None,
    threshold_table_values: dict[str, Any] | None = None,
    variables: dict[str, Any] | None = None,
) -> Any:
    """Округляет значение по типу: decimal, multiple или threshold_table."""
    try:
        if value is None:
            return value

        value = parse_decimal_value(value.replace(",", ".")) if isinstance(value, str) else parse_decimal_value(value)
        ...
        if rounding_type == "threshold_table":
            ...
            if (
                not isinstance(target_variable, str)
                or not isinstance(higher_variable, str)
                or not isinstance(lower_variable, str)
                or not isinstance(formula, str)
                or not target_variable
                or not higher_variable
                or not lower_variable
                or not formula
            ):
            ...
            try:
                ...
                logger.debug("Значения для сравнения:")
                logger.debug(f"{target_variable}: {target_value}")
                logger.debug(f"{formula}: {formula_value}")
                logger.debug(f"{higher_variable}: {higher_value}")
                logger.debug(f"{lower_variable}: {lower_value}")
            ...
            if formula_value < target_value:
                logger.debug(f"formula_value ({formula_value}) < target_value ({target_value})")
                logger.debug(f"Выбрано верхнее значение: {higher_value}")
                return higher_value

            logger.debug(f"formula_value ({formula_value}) >= target_value ({target_value})")
            logger.debug(f"Выбрано нижнее значение: {lower_value}")
            return lower_value

        if rounding_type == "multiple":
            if not rounding_decimal:
                return value
            return _round_to_multiple(value, rounding_decimal)
...
def _normalize_formula_text(formula: str) -> str:
    """Нормализовать текст формулы: подстрочные символы и знаки ×/÷."""
...
def _formula_safe_dict(decimal_vars: dict[str, Decimal]) -> dict[str, Any]:
    """Собрать безопасный namespace для eval формулы (без builtins + Decimal/math)."""
    return {
        "__builtins__": {},
        "Decimal": Decimal,
        "abs": abs,
...
def evaluate_formula(formula, variables, is_condition=False, range_calculation=None, rounding_params=None):
    ...
    try:
        ...
        # Если есть диапазонный расчёт, сразу его применяем
        if not is_condition and range_calculation and "ranges" in range_calculation:
            # Создаем словарь переменных для диапазонного расчёта
            decimal_vars = {}
            for name, value in variables.items():
                try:
                    ...
                    raise ValueError(f"Ошибка преобразования значения {name} = {value} в число: {e!s}") from e
        ...
        # Создаем словарь переменных для обычного расчёта
        decimal_vars = {}
        for name, value in variables.items():
            try:
                new_name = _replace_subscript_digits(name)
        ...
        raise ValueError(f"Ошибка при вычислении формулы '{formula}': {e!s}") from e
...
def calculate_convergence_steps(formula, variables):
    """Вычисляет шаги расчёта повторяемости."""
    try:
