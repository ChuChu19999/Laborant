...
# Константы для расчёта размеров текста
...
def format_decimal_ru(value) -> str:
    ...
    try:
        ...
        if isinstance(value, (int, float)):
            ...
            num_str = str(int(abs_num)) if is_integer else str(abs_num).replace(".", ",")
    ...
    try:
        ...
        for col in range(1, max_col + 1):
            try:
                ...
                writable_cell = as_writable_cell(target_cell)
                if writable_cell is None:
                    continue
                writable_cell.value = source_cell.value
                copy_cell_style(source_cell, writable_cell)
...
def calculate_text_height(text, cell_width_pixels, font_size_pixels=FONT_SIZE_PIXELS):
    """Рассчитывает высоту текста в пикселях с учётом переноса строк."""
    if not text or not isinstance(text, str):
        return LINE_HEIGHT_PIXELS

    try:
        line_height = LINE_HEIGHT_PIXELS * (font_size_pixels / FONT_SIZE_PIXELS)
        ...
        logger.error(f"Ошибка при расчёте высоты текста: {e!s}")
        return LINE_HEIGHT_PIXELS
...
def adjust_cell_height_if_needed(sheet, row, col, text, min_height_pixels=35):
    ...
    try:
        ...
        if required_height_pixels > current_height_pixels:
            new_height_points = min_height_pixels * PIXELS_TO_POINTS

            # Устанавливаем высоту строки
            if row not in sheet.row_dimensions:
                sheet.row_dimensions[row] = RowDimension(sheet, row)
