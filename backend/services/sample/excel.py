...
from services.employee import get_employees_by_hsnils
from services.sample.export import get_samples_export_data
from utils.calculation.export_format import format_sample_calculations_column
from utils.date import parse_datetime_string
...
class SamplesExportPrepared:
    """Подготовленные данные для sync-сборки xlsx экспорта проб."""

    items: list[SampleExportItem]
    employees_map: dict[str, EmployeeResponse]
    total: int


def _format_date(value: Any) -> str:
    """Отформатировать дату для ячейки экспорта проб."""
    if not value:
        return "-"
...
def _format_sampling_location(sample: SampleExportItem) -> str:
    """Собрать текст места отбора, скважины и режима для экспорта."""
    parts: list[str] = []
...
def _format_protocols(protocols: list[SampleProtocolSummary] | None) -> str:
    """Собрать строку номеров протоколов для экспорта."""
    if not protocols:
        return "-"

    formatted = [protocol.formatted_protocol_number for protocol in protocols if protocol.formatted_protocol_number]
...
def _format_selection_conditions(
    ...
    """Собрать текст условий отбора для экспорта."""
    if not conditions:
        return "-"
...
def _apply_cell_style(cell) -> None:
    """Применить стиль ячейки экспорта проб."""
...
def _build_sample_row(
    ...
    """Собрать строку значений экспорта по пробе."""
    added_by_name = "-"
    if sample.added_by:
        employee = employees_map.get(sample.added_by)
        added_by_name = (employee.full_name if employee and employee.full_name else None) or "-"
...
def build_samples_export_workbook(
    ...
    """Собрать xlsx-файл таблицы поступления проб."""
    workbook = Workbook()
...
async def prepare_samples_export_excel(
    db: AsyncSession,
    ...
    """Подготовить данные экспорта проб (без сборки xlsx)."""
    ...
    return SamplesExportPrepared(items=items, employees_map=employees_map, total=total)


def build_samples_export_file(prepared: SamplesExportPrepared) -> tuple[bytes, str]:
    """Собрать xlsx-файл экспорта проб."""
    excel_bytes = build_samples_export_workbook(prepared.items, prepared.employees_map)
    return excel_bytes, "Поступления_проб.xlsx"
