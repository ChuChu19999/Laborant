...
from repositories import laboratory as laboratory_repo, protocol as protocol_repo, sample as sample_repo
...
async def get_protocols(
    ...
    if search_samples:
        sample_ids_for_search = await protocol_repo.get_sample_ids_by_registration_search(db, search_samples)
...
async def create_protocol(db: AsyncSession, protocol_data: ProtocolCreate) -> Protocol:
    ...
    if protocol_data.protocol_template_id:
        if not await get_protocol_template_by_id(db, protocol_data.protocol_template_id, include_deleted=True):
...
async def update_protocol(db: AsyncSession, protocol_id: int, protocol_data: ProtocolUpdate) -> Protocol:
    """Обновить протокол."""
    ...
    if "protocol_template_id" in update_data and update_data["protocol_template_id"] is not None:
        if not await get_protocol_template_by_id(db, update_data["protocol_template_id"], include_deleted=True):
            raise NotFoundError("Шаблон протокола не найден")
...
async def create_protocol_template(db: AsyncSession, template_data: ProtocolTemplateCreate) -> ProtocolTemplate:
    ...
    if template_data.department_id:
        dept = await laboratory_repo.get_department_by_id(db, template_data.department_id)
        if not dept:
...
async def get_protocol_response_data(db: AsyncSession, protocol_id: int) -> ProtocolResponse:
...
async def get_protocol_template_response_data(db: AsyncSession, template_id: int) -> ProtocolTemplateResponse:
...
async def build_protocols_list_response(db: AsyncSession, protocols: list[Protocol]) -> list[ProtocolResponse]:
    ...
    if all_sample_ids:
        calculations = await get_calculations_by_sample(db, sample_ids=list(all_sample_ids), include_deleted=False)
...
async def get_protocol_detail_response(db: AsyncSession, protocol_id: int) -> ProtocolResponse:
    ...
    if protocol.samples:
        ...
        samples_by_id = {sample.id: sample for sample in samples_list}
        calculations = await get_calculations_by_sample(db, sample_ids=protocol.samples, include_deleted=False)
        sample_ids_with_calculations = {calculation.sample_id for calculation in calculations}
