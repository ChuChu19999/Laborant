...
async def get_protocols_by_sample_ids(db: AsyncSession, sample_ids: list[int]) -> dict[int, list[dict[str, Any]]]:
    """Получить протоколы для списка проб."""
    all_protocols, samples_list = await protocol_repo.get_protocols_by_sample_ids(db, sample_ids)
    ...
    result: dict[int, list[dict[str, Any]]] = {sample_id: [] for sample_id in sample_ids}
            ...
            for sample_id in protocol.samples:
                if sample_id in result:
                    ...
                    protocol_dict = {
                        ...
                        "test_protocol_date": (
                            protocol.test_protocol_date.isoformat() if protocol.test_protocol_date else None
                        ),
