...
async def enrich_visibility_scope_labels(
    ...
    if department_ids:
        departments = [
            ...
            for row in await laboratory_repo.get_departments_for_visibility_scope(db, department_ids)
        ]
...
async def validate_visibility_scope_ids(
    ...
    for department_id in department_ids:
        if not await laboratory_repo.get_department_by_id(db, department_id, include_deleted=True):
            raise NotFoundError("Подразделение не найдено")
...
async def validate_role_scopes_ids(
    ...
    for entry in scopes:
        ...
        dept = await laboratory_repo.get_department_by_id(db, department_id, include_deleted=True)
        if not dept:
            raise NotFoundError("Подразделение не найдено")
        if dept.laboratory_id != laboratory_id:
            raise ValidationError("Подразделение должно принадлежать выбранной лаборатории")
