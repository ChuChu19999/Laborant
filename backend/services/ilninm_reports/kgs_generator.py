...
def _unmerge_data_area(
    ws: openpyxl.worksheet.worksheet.Worksheet,
    from_row: int,
    max_col: int,
) -> None:
    """
    Снимает объединение в зоне данных, иначе openpyxl не даёт писать в MergedCell.

    Затрагиваются диапазоны, пересекающие строки от from_row и столбцы 1..max_col.
    """
    to_unmerge = []
    for merged in list(ws.merged_cells.ranges):
        if merged.max_row < from_row:
            continue
...
async def build_kgs_excel(
    ...
    _unmerge_data_area(ws, template_row, max_col)
