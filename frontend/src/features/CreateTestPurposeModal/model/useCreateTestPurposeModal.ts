...
import { useCreateTestPurpose, type TestPurposeFormValues } from '@/entities/TestPurpose';
...
export type UseCreateTestPurposeModalParams = {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void;
  laboratoryId?: number;
...
/** Оркестрация модалки создания цели испытаний. */
export const useCreateTestPurposeModal = ({
  open,
  onClose,
  onSuccess,
  laboratoryId,
  departmentId,
}: UseCreateTestPurposeModalParams) => {
  const createTestPurposeMutation = useCreateTestPurpose();
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      await createTestPurposeMutation.mutateAsync({
      ...
      notify.error(extractErrorMessage(error, 'Не удалось добавить цель испытаний'));
    } finally {
      setLoading(false);
    }
  };
