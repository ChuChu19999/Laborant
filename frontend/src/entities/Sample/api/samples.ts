...
export const samplesApi = {
  ...
  exportSamples: async (
    ...
    try {
      ...
      const filename = filenameFromAxiosHeaders(response.headers, 'Поступления_проб.xlsx');

      const totalHeaderRaw: unknown =
        response.headers['x-export-total'] ?? response.headers['X-Export-Total'];
      const totalHeader =
        typeof totalHeaderRaw === 'string' || typeof totalHeaderRaw === 'number'
          ? String(totalHeaderRaw)
          : '';
