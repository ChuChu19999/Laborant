/** Подписи результатов сходимости. */
...
/** Заменяет abs(...) на |...| для отображения формулы. */
...
/** Форматирует формулу сходимости для отображения пользователю. */
...
/** Убирает дубликаты и пустые строки из списка. */
...
/** Убирает внешние скобки у строки. */
const stripOuterParens = (value: string): string => value.replace(/^\(|\)$/g, '').trim();

/** Собирает шаги сходимости, релевантные для отображения. */
...
/** Объединяет части отображения через разделитель. */
...
/** Возвращает число знаков после запятой в строковом числе. */
...
/** Округляет значение на один знак больше, чем у основного результата. */
...
/** Округляет значение для фракционного состава конденсата. */
...
/** Определяет подпись параллели по описаниям полей карточки. */
export const getCardParallelLabel = (cardFields: { description?: string }[]): string | null => {
