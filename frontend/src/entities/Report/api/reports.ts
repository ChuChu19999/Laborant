import { axiosInstance } from '@/shared/config';
import { filenameFromAxiosHeaders, type PaginatedResponse } from '@/shared/lib/http';

/** Доступные типы отчётов. */
export const REPORT_TYPES = [
  'Количество проб',
  'Физико-химическая характеристика',
  'Результаты КГС',
  'Результаты НКС',
] as const;

/** Тип отчёта. */
export type ReportType = (typeof REPORT_TYPES)[number];

/** Шаблон отчёта. */
export interface ReportTemplate {
  id: number;
  report_type: string;
  file_name: string;
  version: string;
  laboratory_id: number;
  department_id?: number;
  laboratory_name?: string;
  department_name?: string;
  created_at: string;
  updated_at: string;
  deleted_at?: string;
}

/** Данные для создания шаблона отчёта. */
export interface ReportTemplateCreate {
  report_type: string;
  file_name: string;
  file: string;
  laboratory_id: number;
  department_id?: number;
}

/** Данные для обновления шаблона отчёта. */
export interface ReportTemplateUpdate {
  report_type?: string;
  file?: string;
  file_name?: string;
}

async function postReportBlob(
  url: string,
  params: Record<string, unknown>,
  fallbackFilename: string
): Promise<{ blob: Blob; filename: string }> {
  const response = await axiosInstance.post<Blob>(url, params, { responseType: 'blob' });
  return {
    blob: response.data,
    filename: filenameFromAxiosHeaders(response.headers, fallbackFilename),
  };
}

/** API отчётов. */
export const reportsApi = {
  getReportTemplates: async (
    laboratoryId?: number,
    departmentId?: number,
    page?: number,
    pageSize?: number
  ): Promise<PaginatedResponse<ReportTemplate>> => {
    const params: Record<string, unknown> = {};
    if (laboratoryId) {
      params.laboratory_id = laboratoryId;
    }
    if (departmentId) {
      params.department_id = departmentId;
    }
    if (page !== undefined) {
      params.page = page;
    }
    if (pageSize !== undefined) {
      params.page_size = pageSize;
    }
    const response = await axiosInstance.get<PaginatedResponse<ReportTemplate>>(
      '/api/report-templates/',
      { params }
    );
    return response.data;
  },

  getAvailableReportTemplates: async (
    laboratoryId: number,
    departmentId?: number
  ): Promise<ReportTemplate[]> => {
    const params: Record<string, unknown> = {
      laboratory_id: laboratoryId,
    };
    if (departmentId) {
      params.department_id = departmentId;
    }
    const response = await axiosInstance.get<ReportTemplate[]>('/api/report-templates/available/', {
      params,
    });
    return response.data;
  },

  getReportTemplate: async (id: number): Promise<ReportTemplate> => {
    const response = await axiosInstance.get<ReportTemplate>(`/api/report-templates/${id}/`);
    return response.data;
  },

  createReportTemplate: async (data: ReportTemplateCreate): Promise<ReportTemplate> => {
    const response = await axiosInstance.post<ReportTemplate>('/api/report-templates/', data);
    return response.data;
  },

  updateReportTemplate: async (id: number, data: ReportTemplateUpdate): Promise<ReportTemplate> => {
    const response = await axiosInstance.patch<ReportTemplate>(
      `/api/report-templates/${id}/`,
      data
    );
    return response.data;
  },

  getReportTemplateFile: async (templateId: number): Promise<Blob> => {
    const response = await axiosInstance.get<Blob>(`/api/report-templates/${templateId}/`, {
      params: {
        download: true,
      },
      responseType: 'blob',
    });
    return response.data;
  },

  generateSampleCountReport: async (params: {
    laboratory_id: number;
    department_id?: number;
    template_id?: number;
    date_from: string;
    date_to: string;
  }): Promise<{ blob: Blob; filename: string }> => {
    return postReportBlob(
      '/api/report-templates/generate/sample-count/',
      params,
      `Количество_проб_${params.date_from}_${params.date_to}.zip`
    );
  },

  generatePhysicochemicalReport: async (params: {
    laboratory_id: number;
    department_id?: number;
    template_id?: number;
    date_from: string;
    date_to: string;
    sampling_location: string;
  }): Promise<{ blob: Blob; filename: string }> => {
    return postReportBlob(
      '/api/report-templates/generate/physicochemical-characteristic/',
      params,
      `Физико_химическая_характеристика_${params.sampling_location}_${params.date_from}_${params.date_to}.xlsx`
    );
  },

  generateNksReport: async (params: {
    laboratory_id: number;
    department_id?: number;
    template_id?: number;
    date_from: string;
    date_to: string;
    report_month: number;
    report_year: number;
  }): Promise<{ blob: Blob; filename: string }> => {
    return postReportBlob(
      '/api/report-templates/generate/nks-results/',
      params,
      `Результаты_НКС_${params.report_month}_${params.report_year}_${params.date_from}_${params.date_to}.xlsx`
    );
  },

  generateKgsReport: async (params: {
    laboratory_id: number;
    department_id?: number;
    template_id?: number;
    date_from: string;
    date_to: string;
  }): Promise<{ blob: Blob; filename: string }> => {
    return postReportBlob(
      '/api/report-templates/generate/kgs-results/',
      params,
      `Результаты_КГС_${params.date_from}_${params.date_to}.xlsx`
    );
  },
};
