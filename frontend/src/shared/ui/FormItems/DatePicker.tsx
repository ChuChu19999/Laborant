    ...
    for (let i = 0; i < formatted.length; i++) {
      if (/\d/.test(formatted[i] ?? '')) {
        ...
        if (digitCount === targetDigitsCount) {
          newPosition = i + 1;
          // Если следующая позиция - точка, пропускаем её
...
const DatePicker = (props: DatePickerProps) => {
  ...
  return (
    <ConfigProvider locale={ruRU}>
      <div ref={containerRef} className="form-item-control">
        <AntDatePicker
          className={['form-item-control', className || 'date-picker'].filter(Boolean).join(' ')}
...
const RangePicker = (props: RangePickerProps) => {
  ...
  return (
    <ConfigProvider locale={ruRU}>
      <div ref={containerRef} className="form-item-control">
        <AntRangePicker
          className={['form-item-control', props.className || 'date-range-picker']
            .filter(Boolean)
            .join(' ')}
          format={format || 'DD.MM.YYYY'}
          inputReadOnly={inputReadOnly ?? false}
